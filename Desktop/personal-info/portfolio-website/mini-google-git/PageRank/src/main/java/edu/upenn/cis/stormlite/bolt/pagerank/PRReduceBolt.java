...
public class PRReduceBolt implements IRichBolt {
    ...
    public void prepare(Map<String,String> stormConf, TopologyContext context, OutputCollector collector) {
        ...
        String reducePath = WorkerAdmin.dbStore + "/" + WorkerAdmin.reducePrefix + executorId;
    	if(!Files.exists(Paths.get(reducePath))) {
    		try {
				Files.createDirectories(Paths.get(reducePath));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        ...
        String resPath = WorkerAdmin.dbStore +  "/" + "Worker" + stormConf.get("workerIndex") + "/" + WorkerAdmin.resPrefix + executorId;
        // String resPath = WorkerAdmin.dbStore +  "/" + "Worker" + stormConf.get("workerIndex") + "/" + WorkerAdmin.resPrefix + executorId;
        createDir(resPath);
    ...
    public synchronized boolean execute(Tuple input) {
    	...
    	if (sentEof) {
			...
			if (this.eosChecker.addVoterAndCheckEos(input.getSourceExecutor(), "ReduceBolt") && !writtenToBolt ) {
				...
				Context outputContext = new Context() {
					...
					public void write(String key, String value) {
						...
						// System.out.println("[ReduceBolt]: write to printBolt:  " + key + "," + value+ ". "+ getExecutorId());
    		...
    		System.out.println("[ReduceBolt_]: read :  { " + key + " , " + value +  "}. " + getExecutorId());
