...
public class SenderBolt implements IRichBolt {
    ...
    private synchronized void send(Tuple tuple) throws IOException {
        ...
        System.out.println("Sender is routing " + tuple.getStringByField("url") + " to " + address + "/" + stream);
        		...
        		os.write(toSend);
        		os.flush();
        		if(conn.getResponseCode()!=200)throw new RuntimeException("distributed work connection error, please check error");
                conn.disconnect();
    }
