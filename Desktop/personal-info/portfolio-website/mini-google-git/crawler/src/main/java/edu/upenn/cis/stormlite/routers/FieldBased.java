...
public class FieldBased extends StreamRouter {
	...
	public IRichBolt getBoltFor(List<Object> tuple) {
		...
		for (Integer i: fieldsToHash) {
			url = (String)tuple.get(i);
			if(url.startsWith("https://") || url.startsWith("http://")) {
				try{
				//	URLInfo info = new URLInfo(url);
				//	String hostname = info.getHostName();
					//hash ^= (hostname+info.getFilePath()).hashCode();
					hash ^= (CrawlerUtil.generateURLId(url)).hashCode();
				} catch (Exception e) {
		...
		if (hash < 0)
			hash = hash + getBolts().size();
		//URLDistributeBolt b = (URLDistributeBolt)(getBolts().get(hash));
		//System.out.println("***Sending**** to hash:["+(hash+1)+ "th]***bolts:----"+ b.workerIndex +"----"+url);
		//System.out.println("[total bolt size is "+getBolts().size()+"]");
