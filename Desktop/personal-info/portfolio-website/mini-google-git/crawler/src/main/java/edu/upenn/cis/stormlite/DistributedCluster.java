...
public class DistributedCluster implements Runnable {
	...
	public DistributedCluster (int numThreads) {
		...
		log.info("distributed cluster instance created with size : #"+numThreads);
	}
...
//		System.out.println("bolts size are: "+ boltStreams.size());
//		for(Map.Entry<String, List<IRichBolt>> entry : boltStreams.entrySet()) {
//			System.out.print("bolts key is: ");
//		}
		return context;
	}
		...
		for (String key: topo.getBolts().keySet()) {
			...
			for (int i = 0; i < localExecutors; i++)
				try {
					...
					System.out.println("Worker Index[" + WorkerServer.index + "], BoltIndex["+(boltStreams.get(key).size()-1)+"] Created a bolt executor " + key + "/" + newBolt.getExecutorId() + " of type " + bolt.getLeft());
				} catch (InstantiationException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (IllegalAccessException e) {
		...
		//	int count = boltStreams.get(stream).size();
			...
			for (String worker: WorkerHelper.getWorkers(config)) {
				...
				if (workerId++ != Integer.valueOf(config.get("workerIndex"))) {
					if(stream.equals("URL_DISTRIBUTE_BOLT")) {
