...
public class DistributedCluster implements Runnable {
	...
	ExecutorService executor = Executors.newFixedThreadPool(5); // we'll do a single threaded pool to avoid races
																// between EOS propagation and tuple propagation!

//	ExecutorService spoutExecutor = Executors.newFixedThreadPool(1);

	Queue<Runnable> taskQueue = new ConcurrentLinkedQueue<Runnable>();

	/**
	 * set executor thread number
	 * 
	 * @param numThreads
	 */
	public void setThreadNum(int numThreads) {
		this.executor = Executors.newFixedThreadPool(numThreads);
		log.info("New DistributedCluster thread number created. Thread number:" + numThreads);
	}
	...
	public TopologyContext submitTopology(String name, Config config, Topology topo) throws ClassNotFoundException {
		...
		// scheduleSpouts(); Moving this to run() method below so that tasks are not
		// queued until job starts running
...
//				if (task.getClass().equals(SpoutTask.class)) {
//					spoutExecutor.execute(task);
//				} else {
	...
	private void createRoutes(Topology topo, Config config) {
		...
		for (String stream : topo.getBolts().keySet()) {
			...
			for (String worker : WorkerHelper.getWorkers(config)) {
				...
				if (workerId++ != Integer.valueOf(config.get("workerIndex"))&& stream.equals("REDUCE_BOLT")) {        // assign only the reduce bolts have senderbolts routers
